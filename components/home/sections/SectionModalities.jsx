...
function SectionModalities({ isActive }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setTextReady(true), 550);
    return () => clearTimeout(t);
  }, [isActive, isMobile]);

  useEffect(() => {
    if (!isActive) { setListOpen(false); }
  }, [isActive]);
  ...
  return (
    <section className="home-section hs-grid atm-cream" id="s-modalities" data-section="1">
      <div className="hs-img hs-img-l">
        <DaVinciPanel isActive={isActive} variant="mod" animKey={animKey} hoveredId={isMobile ? null : hoveredId} />
      </div>
      <div className="hs-txt hs-txt-r">
        {isMobile ? (
          /* ── Mobile: compact overlay text + collapsible list ── */
          <div className={`home-section__content mob-txt-delayed${textReady ? ' ready' : ''}`}>
            <span className="eyebrow"><span className="dot" />What we read · 01 / 05</span>
            <div className="home-section__hgroup">
              ...
              <p className="home-section__lede">
                Labs, imaging, genomics, medications and behaviour —
                one picture.
              </p>
            </div>
            <button
            ...
            >
              {listOpen ? '↑ Less' : '↓ Explore signals'}
            </button>
            <div className={`mob-list-panel${listOpen ? ' mob-list-open' : ''}`}>
              <div className="mods-mini">
                {mods.map(m => (
                  <div className="mm" key={m.num}>
